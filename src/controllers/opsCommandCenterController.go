...
var opsFleetRegistry = []opsFleetMember{
	...
	{Key: "retention", Label: "Retention", Family: "platform", Kind: "autopilot", CockpitPath: "/platform/retention", Lanes: []opsFleetLane{{Key: "database", Label: "Database custody", Table: "retention_policies", TenantColumn: "tenant_id", PauseColumn: "paused_until", Scheduled: true, CockpitPath: "/platform/retention", StatusSQL: "SELECT tenant_id, enabled, mode, paused_until, last_run_at, schedule_interval_minutes AS interval_minutes, created_at FROM retention_policies"}}},
...
func opsOpenAttention(db *gorm.DB, now time.Time) ([]opsAttentionItem, []string) {
	...
	appendRows("retention", "approval", "/platform/retention", "SELECT tenant_id, public_id::text AS native_id, 'major' AS severity, action_class AS title, COALESCE(NULLIF(guardrail,''), 'Retention action requires approval') AS detail, 1 AS count, created_at AS first_seen FROM retention_actions WHERE outcome='approval_required'")
